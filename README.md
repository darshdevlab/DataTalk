...
first page is a schema/data explorer with an ERD-style relationship map where
users can inspect tables, columns, relationships, glossary terms, and sample
rows. The second page is a chat-style query interface that runs the supported
intent compiler in browser JavaScript using the same embedded demo rows, so
